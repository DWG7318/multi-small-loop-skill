...
def validate_assets(questions: dict[str, Any], key: dict[str, Any]) -> None:
    if questions.get("mode") != "MSLK" or key.get("mode") != "MSLK":
...
def _metadata_error(metadata: dict[str, Any], seed: int) -> str | None:
    ...
    if role in {"checker", "worker"} and not pair_id:
        return "missing_pair_id"
    if role == "supervisor" and pair_id is not None:
        return "unexpected_pair_id"
...
def grade(
    ...
    return {
        "receipt_type": "MSLK_READINESS_EVAL_RECEIPT",
        "result": "MSLK_READINESS_EVAL_PASS" if passed else "MSLK_READINESS_EVAL_FAIL",
...
def verify_receipt(
    ...
    checks = {
        ...
        "pair_id": receipt.get("pair_id") == metadata.get("pair_id"),
...
def _role_key(item: dict[str, Any]) -> tuple[str | None, str | None]:
    return item.get("candidate_role"), item.get("pair_id")
...
def verify_roster(
    ...
    if len(set(expected_keys)) != len(expected_keys):
        return {"valid": False, "reason": "duplicate_roster_role"}
    if expected_keys.count(("supervisor", None)) != 1:
        return {"valid": False, "reason": "invalid_supervisor_count"}
    pair_ids = {pair_id for role, pair_id in expected_keys if role != "supervisor"}
    if len(pair_ids) < 2:
        return {"valid": False, "reason": "insufficient_pairs"}
    for pair_id in pair_ids:
        if {role for role, value in expected_keys if value == pair_id} != {
            "checker",
            "worker",
        }:
            return {"valid": False, "reason": "incomplete_pair"}
    conversations = [item.get("conversation_id") for item in expected]
    if len(set(conversations)) != len(conversations):
        return {"valid": False, "reason": "duplicate_conversation"}
    receipt_map = {_role_key(item): item for item in receipts}
    if set(receipt_map) != set(expected_keys):
        return {"valid": False, "reason": "roster_identity_mismatch"}
    expected_map = {_role_key(item): item for item in expected}
    checks = {
        f"{role}:{pair_id or 'project'}": verify_receipt(
            receipt_map[(role, pair_id)], expected_map[(role, pair_id)], root=root
        )
        for role, pair_id in sorted(expected_keys, key=lambda item: (item[0] or "", item[1] or ""))
    }
    return {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the MSLK readiness evaluation.")
    ...
    grade_parser.add_argument("--pair-id")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "grade":
        ...
        return 0 if receipt["result"] == "MSLK_READINESS_EVAL_PASS" else 1
